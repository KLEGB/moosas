...
def _default_path_element_dict(path_like=None, win_type=1, prj_index=0, user_name="", path_height=0.0, path_width=0.0,
    ...
    result = {
        ...
        "lam": lam,
        "turb": turb,
        "expt": 0.650152978 if dtype == "plr_leak3" else 0.5,
    }
...
def _normalize_path_element_data(raw_value, *, path_height=0.0, path_width=0.0, win_type=1, prj_index=0, user_name="",
    ...
    if isinstance(raw_value, dict):
        ...
        element = {}
    requested_operable = operable if operable is not None else element.get("operable", 1.0)
    ...
    element["operable"] = _coerce_operable(element.get("operable", requested_operable), defaults.get("operable", 1.0))
    if not dtype:
...
def _load_network_payload(networkFilePath: str) -> dict:
    with open(networkFilePath, "r", encoding="utf-8") as file:
        return _normalize_network_payload_data(json.load(file))
...
class AfnPath(object):
    """
        ...
        element: serializable CONTAM flow-element parameters
        geometry: reference MoosasElement (optional)
    """
    ...
    def fromElement(cls, moGeometry: MoosasGlazing | MoosasSkylight, pathName=None, fromZone=None, toZone=None,
        ...
        # Preserve the optical property in network snapshots.  The in-memory
        # Coupler normally has ``geometry`` available; this makes a JSON/LPG
        # round trip equally capable of calculating transmitted solar heat.
        thePath["element"]["shgc"] = float(moGeometry.SHGC or 0.0)
...
def applyWindPressure(pathList: list[AfnPath], windVector: Vector, speed: float = None,
    ...
    if not geometry_paths:
        ...
        return pathList
    xgbInput = [pressureInput(windVector, path_item.geometry) for path_item in geometry_paths]
    Wp = callXgb(xgbInput)

    pressure = Wp * airDensity * windVector.length(power=True) * np.power(
        [path_item.position_z for path_item in geometry_paths], alpha * 2
    ) / 2
    for path_item in pathList:
        path_item.pressure = 0.0
    for _path, _pressure in zip(geometry_paths, pressure):
        if Vector.parallel(_path.orientation, [0, 0, 1]):
            _pressure = 0
...
def buildNetworkFile(model=None, pathList: list[AfnPath] = None, zoneList: list[AfnZone] = None,
    ...
    """
        Build a JSON airflow-network file from model or pathList/zoneList.
    ...
    if networkDict is not None:
        ...
        if networkFilePath is None:
            networkFilePath = os.path.join(tempfile.mkdtemp(prefix="moosas-afn-"), "network.json")
...
def buildPrj(model=None, pathList: list[AfnPath] = None, zoneList: list[AfnZone] = None,
    ...
    networkFilePath : str, optional
        File path for the generated JSON network input for MoosasAFN. If None, defaults to a temporary location.
    ...
    """
        ...
        Command line should be: MoosasAFN.exe [-h,-p...] inputNetworkFile.json
        Optional command:
    ...
    if networkFilePath is None:
        ...
        networkFilePath = os.path.join(workspace, prjTempName + '.json')
...
def buildZoneInfoFile(model=None, zoneList: list[AfnZone] = None, networkFilePath=None, pathList: list[AfnPath] = None,
    ...
    networkFilePath : str, optional
        Path to the JSON network file. If not provided, a temporary file will be generated using `buildNetworkFile()`.
    pathList : list of AfnPath, optional
    ...
    if payload is None and networkFilePath is None:
        prjTempName = 'afn_' + generate_code(4)
        networkFilePath = os.path.join(tempfile.mkdtemp(prefix="moosas-afn-"), prjTempName + '.json')
        if zoneList is None:
            if model is None:
                raise Exception("model, pathList and zoneList cannot be all None")
            zoneList, pathList = getZoneAndPath(model)
