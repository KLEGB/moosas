...
def estimate_curtainwall_cost(
    ...
    def choose_grid(n_sashes: int) -> Tuple[int, int, bool]:
        ...
        for rows in range(r0, r_max + 1):
            ...
            key = (T, cols, -rows)
            if best_key is None or T < best_key[0]:
                best_key = key
                best_rc = (rows, cols)
    ...
    def opening_cost(height,width)->float:
        return (height+width)*28*2.0+240 + height*width * prices.get("window_per_m2", 200)
    # ------------------------
    # Step 1: Initial n, then iterate until sash area fits inside unit area
    # ------------------------
    rows, cols = _ceil(H / mH), 1  # number of rows and columns
    unitH, unitW = H / rows, mW  # the height or width for a grid
    if S <= 1e-12:
        n = 0
    else:
        n = _ceil(S / (unitH * mW))
    ...
    waste = waste or {"profile": 0.06, "glass": 0.02, "window": 0.03}
    profile_per_m = prices.get("profile_per_m", None)
    profile_per_unit = prices.get("profile_per_unit", None)

    glass_fixed_per_m2 = float(prices.get("glass_fixed_per_m2", 184.59))
    ...
    install_per_m2 = float(prices.get("install_per_m2", 166.41))

    # Waste factors
    w_profile = float(waste.get("profile", 0.06))
    w_glass = float(waste.get("glass", 0.03))
    w_window = float(waste.get("window", 0.02))
    ...
    if profile_per_m is not None:
        ...
        profile_cost_base = cols * rows * float(profile_per_unit)
    ...
    if n <= 0:
        ...
        if window_pricing == "per_m2":
            ...
            window_cost_base = opening_cost(unitH,unitW) * n
...
if __name__ == "__main__":
    # Baseline unit prices (RMB) for demonstration
    # Replace with your contracted/quoted prices in real projects.
    prices_demo = {
        "profile_per_m": 220.0,  # RMB/m (assumed)
        "glass_fixed_per_m2": 190.0,  # RMB/m² (example)
        "window_pricing": "per_m2",
